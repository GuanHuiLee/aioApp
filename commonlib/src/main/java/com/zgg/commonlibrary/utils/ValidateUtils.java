...
public class ValidateUtils {
    ...
    //手机号表达式
    private final static Pattern phone_pattern = Pattern.compile("^((13[0-9])|(15[0-9])|(18[0-9])|(14[7])|(17[0|3|6|7|8]))\\d{8}$");
    ...
    // 验证密码必须包含字母和数字,6到20位
    public static final String REG_PASSWORD_COMMON = "(?=^.{6,20}$)((?=.*\\d)((?=.*[A-Z])|(?=.*[a-z]))|(?=.*[A-Z])((?=.*\\d)|(?=.*[a-z]))|(?=.*[a-z])((?=.*[A-Z])|(?=.*\\d)))^.*";
...
//^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$
    ...
    public static boolean isIDCard(String IDCardNo) {
        ...
        try {
            ...
            e.printStackTrace();
            errmsg = "身份证生日不在有效范围";
            Logger.e(errmsg + e.getMessage());
            return false;
        } catch (java.text.ParseException e1) {
            e1.printStackTrace();
            errmsg = "身份证生日不在有效范围";
            Logger.e(errmsg + e1.getMessage());
            return false;
        }
    ...
    /**
     * 验证密码必须在6到20位，且必须包含字母和数字
     * @param pass
     * @return
     */
    public static boolean validatePasswd(String pass) {
        return !TextUtils.isEmpty(pass) && pass.matches(REG_PASSWORD_COMMON);
    }
